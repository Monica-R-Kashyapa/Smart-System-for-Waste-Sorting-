...
# YOLOv5 model path
model_path = "C:/Users/Monica/Desktop/w/waste.v3i.yolov5pytorch/yolov5/waste-sorte/exp/weights/best.pt"
...
def real_time_processing():
    global process
    try:
        # Start the real-time processing script
        process = subprocess.Popen(['python', 'C:/Users/Monica/Desktop/w/waste.v3i.yolov5pytorch/yolov5/waste-sorte/r.py'])
        return redirect(url_for('detect_waste'))
    except Exception as e:
        return f"Error starting real-time processing: {str(e)}", 500
...
def stop_processing():
    ...
    try:
        if process and process.poll() is None:  # Check if the process is running
            ...
            return redirect(url_for('rindex'))   # Redirect to the main page
    except Exception as e:
        print(f"Error stopping real-time processing: {e}")
        return render_template('error.html', error_message=f"Error stopping real-time processing: {e}")
